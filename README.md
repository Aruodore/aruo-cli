...
**A CLI that scaffolds production-ready software projects (CI, governance, security policy, and documentation from the first commit) and audits any repository's engineering health against a versioned policy.**

> **Status: [v0.1.0](https://github.com/Aruodore/aruo-cli/releases/tag/v0.1.0), pre-1.0, marked as a pre-release.** `aruo create` and `aruo doctor` are the two commands used throughout this document. Every output shown below was run against the current checkout. The repository is public and the first tagged release is out, with prebuilt binaries; see [Installation](#installation). Expect breaking changes before 1.0.
...
Most generators stop after copying files, so the CI workflow, security policy, and license drift out of sync the moment a project starts changing. Aruo treats creation as the beginning, not the whole job: every generated project ships with a working CI pipeline, governance and security files, and tests that run. `aruo doctor` can later re-score any repository, generated by Aruo or not, against the same versioned policy.
...
That's install → verify → create → inspect the result, using only commands that exist. `aruo create` also runs fully interactively (just run `aruo create` with no flags) and lets you go back to any earlier screen with Shift+Tab, or by typing `back` in the accessible fallback.
