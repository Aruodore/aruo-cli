...
- Application template IDs `react-lean`, `vue-lean`, `next-lean`, and `nuxt-lean` were removed. Use `react`, `vue`, `next`, and `nuxt` respectively.
- The unsuffixed application templates now generate the former focused baselines. The comprehensive database, container, health-check, and operational scaffolds are no longer installed automatically. Add only the capabilities the application requires and record their status in `aruo.yaml`.
- Newly initialized and generated repositories receive managed contract version 2. Stack-specific template guidance now lives in application-owned `AGENTS.local.md`; `AGENTS.md` remains Aruo-managed. Existing repositories are not rewritten, and managed contract updates remain deferred until `aruo update` is implemented.

#### Migration

No migration is required for repositories already generated by Aruo. For scripts that invoke `aruo create`, replace each `*-lean` template ID with its unsuffixed ID. Do not copy the removed comprehensive scaffolding forward unless the application has a demonstrated need for those capabilities.
